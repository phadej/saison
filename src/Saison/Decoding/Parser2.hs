...
module Saison.Decoding.Parser2 where
...
type Parser tk k = ByteString -> Stack String k -> tk k String

data Stack e k where
    Bottom     ::              Stack e ByteString
    PushArray  :: Stack e k -> Stack e (TkArray k e)
    PushRecord :: Stack e k -> Stack e (TkRecord k e)
...
tokens bs0 = goT bs0 Bottom where
    goT :: Parser Tokens k
    ...
    popStack :: Stack String k -> ByteString -> k
    popStack Bottom         bs = bs
    popStack (PushArray k)  bs = goA1 bs k
    popStack (PushRecord k) bs = goR1 bs k

    tokenCase
        :: Word8              -- head
        -> ByteString         -- tail
        -> ByteString         -- whole input, needed for number parsing
        -> Stack String k     -- continuation
        -> Tokens k String
    ...
    tokenCase 34  {- " -} bs _   k = case Atto.parse jstring_ bs of
        Atto.Done bs1 t   -> TkText t (popStack k bs1)
    ...
    tokenCase w           _  wbs k
        | 48 <= w && w <= 75 || w == 45 = case attoParse scientific wbs of
            Atto.Done bs1   s -> TkNumber s (popStack k bs1)
    ...
    tokenCase 110 {- n -} bs _   k
        | Just bs1 <- stripPrefix "ull" 3 bs = TkLit LitNull (popStack k bs1)
    tokenCase 116 {- t -} bs _   k
        | Just bs1 <- stripPrefix "rue" 3 bs = TkLit LitTrue (popStack k bs1)
    tokenCase 102 {- f -} bs _   k
        | Just bs1 <- stripPrefix "alse" 4 bs = TkLit LitFalse (popStack k bs1)
    ...
    goA bs' k = case BS.uncons bs of
        ...
        Just (93, !bs1) -> TkArrayEnd (popStack k bs1)
        Just (w,  !bs1) -> TkItem $ tokenCase w bs1 bs $ PushArray k
      where
        bs = skipSpace bs'
    ...
    goA1 bs' k = case BS.uncons bs of
        Nothing         -> tkErrEOF ", or ]"
        Just (93, !bs1) -> TkArrayEnd (popStack k bs1)
        Just (44, !bs1) -> TkItem $ goT bs1 $ PushArray k
        _               -> tkErrBS bs ", or ]"
      where
        bs = skipSpace bs'
    ...
    goR bs k = case uncons bs of
        ...
        Just (125, !bs1) -> TkRecordEnd (popStack k bs1)  -- }
    ...
    goRK bs1 k = case Atto.parse jstring_ bs1 of
        Atto.Done bs2 t   -> case uncons bs2 of
            Nothing         -> tkErrEOF ":"
            Just (58, !bs3) -> TkPair t $ goT bs3 $ PushRecord k
            Just _          -> tkErrBS bs2 ":"
